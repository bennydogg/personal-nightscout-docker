...
- **nightscout** — `nightscout/cgm-remote-monitor:15.0.3` (internal port 1337, mapped to `HOST_PORT`)
- **mongo** — `mongo:4.4` with WiredTiger cache capped at 0.25GB per instance
...
| `backup.sh` | MongoDB backup (see known issues below) |
| `lib/instance-utils.sh` | Shared instance discovery functions |
...
# Test MongoDB (mongo:4.4 uses legacy 'mongo' shell, NOT mongosh)
...
- **MongoDB 4.4 is EOL** (Feb 2024). Upgrade to 7.0 planned but requires stepping through 5.0/6.0.
- **`backup.sh`** needs work: `mongodump` is missing auth flags, interactive prompts hang in cron, references non-existent `restore.sh`.
- **`mongo:4.4` uses the legacy `mongo` shell**, not `mongosh`. All exec commands must use `mongo`.

## Version Information

- **Nightscout:** 15.0.3 (pinned)
- **MongoDB:** 4.4 (EOL — upgrade planned)
